...
* UNION : 여러 쿼리 블록의 결과를 하나의 결과 집합으로 결합   
=> 두 쿼리 블록의 모든 결과를 결합하고, 중복 제거.   
...
<연산자 우선순위>   
* INTERSECT : UNION, EXCEPT보다 먼저 평가됨   
* UNION, INTERSECT : 교환법칙 성립   
* EXCEPT : 교환법칙 불가   
...
- 시간형(temporal) 주의      
> SUM(), AVG()는 시간형에 직접 동작하지 않음(숫자로 변환되어 비의도적 손실)

> 별도 언급이 없으면, 집계 함수는 NULL 값을 무시
> GROUP BY가 없는 문장에서 집계 함수를 사용하면 “모든 행을 하나의 그룹으로 집계”한 것과 같음
