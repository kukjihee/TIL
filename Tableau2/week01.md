...
<img width="822" height="740" alt="화면 캡처 2025-09-08 195419" src="https://github.com/user-attachments/assets/4eb959b7-9d4a-4b21-985b-511d38abb937" />
...
```
* 라이브 연결은 원본 데이터베이스와 실시간으로 연결되어 데이터가 변경되면 시각화에도 즉시 반영됨.
* 추출 연결은 데이터를 Tableau 전용 파일로 복사하여 사용하는 방식=> 원본 DB에 계속 연결할 필요가 없어 속도가 빠르고, 오프라인 환경에서도 분석이 가능
대신 원본 데이터가 바뀌더라도 자동으로 반영되지 않기에 최신 데이터를 보려면 리프레시 필요
```
...
```
* 관계: 서로 다른 수준의 데이터를 유연하게 연결 → 데이터 손실 위험 줄임.   
* 조인: 같은 수준의 데이터를 결합할 때 → 분석용 통합 테이블 생성.
```
...
<img width="882" height="730" alt="화면 캡처 2025-09-08 200846" src="https://github.com/user-attachments/assets/f726b1d9-cd67-48b4-9ff2-d68b560c280d" />
...
데이터 혼합     
: 시트에서 보조 데이터 필드를 끌어다 쓰면, 자동으로 데이터 혼합 발생  
=> 데이터 소스가 서로 다른 경우 사용   
...
유니온   
: 동일한 구조(컬럼 이름/컬럼 수)를 가진 테이블이나 시트를 세로 방향으로 합치는 것   


> **🧞‍♀️ 유니온을 사용하기 위한 전제 조건은 무엇인가요?**
> 필드의 이름과 개수, 데이터 유형이 동일해야 함
...
```
실시간 주식 시세는 데이터가 매 순간 변동하므로, 항상 최신 정보가 반영되어야 하므로 
라이브 연결을 사용하는 것이 적합.   
```
...
```
데이터를 미리 다운로드하여 오프라인에서도 사용할 수 있는 추출을 사용하는 것이 적합
```
